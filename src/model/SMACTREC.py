        ...
        with tf.variable_scope("CSTMA"):
            ...
            for i in range(FLAGS.num_blocks):
                with tf.variable_scope("num_blocks_%d" % i):
                    attention = T.SelfModulatingAttention(self.num_units, self.num_heads, self.num_events,
    ...
    def train(self, features, labels):
        ...
        if self.ct_reg != 0.:
            ...
            for seqs_intny in tf.get_collection("LLE_PP"):
                ct_regularizer = T.SelfModulatingAttention.biased_likelihood(
                    seqs_intny, next_mark_onehot, seqs_spans, self.num_heads)
